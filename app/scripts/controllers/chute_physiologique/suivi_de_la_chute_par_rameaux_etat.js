...
/**
 * @ngdoc function
 * @name beeOneWebFrontApp.controller:ChutePhysiologiqueSuiviDeLaChuteParRameauxEtatCtrl
 * @description
 * # ChutePhysiologiqueSuiviDeLaChuteParRameauxEtatCtrl
 * Controller of the beeOneWebFrontApp
 */
angular.module('beeOneWebFrontApp')
  .controller('ChutePhysiologiqueSuiviDeLaChuteParRameauxEtatCtrl', function($q, translatedwords, ChutePhysiologique, campagneagricole, $window, $translatePartialLoader, $translate, societe, $state, _url, $mdDialog, VarieteService, parcellecultural, cultureService, $cookies, portGreffe, FermeService, $scope, bilanHydrique, ApportEau, getsuperficie, savefilter) {
    var pc = this;
    ...
    $scope.search = () => {
      ...
      NProgress.start();
      pc.ChuteRameaux_etat = []
      $q.all([ChutePhysiologique.getAllChuteRameaux_etat(pc.obj)]).then((values) => {

        pc.ChuteRameaux_etat = values[0].data;
    ...
    pc.printExcel = (e) => {
      $("#tableDataShow").table2excel({
        filename: "Etat de synthèse Suivi de la chute par rameaux.xls",
        preserveColors: true
      });
    };
