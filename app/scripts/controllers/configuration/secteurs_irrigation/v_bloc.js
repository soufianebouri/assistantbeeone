...
/**
 * @ngdoc function
 * @name beeOneWebFrontApp.controller:ConfigurationSecteursIrrigationVBlocCtrl
 * @description
 * # ConfigurationSecteursIrrigationVBlocCtrl
 * Controller of the beeOneWebFrontApp
 */
angular.module('beeOneWebFrontApp')
  .controller('ConfigurationSecteursIrrigationVBlocCtrl', function (
    $q,
    $scope,$mdDialog,
    toastr,
    ...
    vm.modifier = async function  () {
        ...
        if(await vm.validateFormData()){
          ...
          Bloc.edit(vm.formData).then(async e => {

              toastr.clear();
              toastr.success('Bloc bien modifé', {
                closeButton: true
              });
              NProgress.done();
    ...
    vm.validateFormData = async function() {
          ...
          let rules = {
            ...
            BLOC: "BLOC is required.",
            Stat_Tete: "Station is required."
          };
    ...
    vm.ajouter = async function  () {
      toastr.clear();
        if(await vm.validateFormData()){
          NProgress.start()
          Bloc.add(vm.formData).then(async e => {
              toastr.clear();
              toastr.success('Bloc bien ajouté', {
                closeButton: true
              });
              await $scope.undoSelect()
              NProgress.done();
              vm.data_bloc.unshift(e.data);
              vm.dtInstance.reloadData();
              vm.reset();
          }).catch(async e => {
            NProgress.done();
      ...
      vm.multiDelete = async function() {
        ...
        let selectedIds = await $scope.getSelectedIDs(vm.data_bloc);
          ...
          onShown: function(toast) {
            ...
            $("#confirmationRevertYes").click(function() {
              ...
              Bloc.multidelete({
                ...
                await $scope.undoSelect()
                toastr.clear();
                toastr.success("Bloc(s) successfully deleted.", {
                  closeButton: true
                });
                vm.data_bloc = vm.data_bloc.filter(item => !selectedIds.includes(item.ID));
                vm.dtInstance.reloadData();
                NProgress.done();
    ...
    vm.delete = async function(data) {
      ...
      toastr.error("<button type='button' id='confirmationRevertYes' class='btn btn-danger' style='float : right;'>Je confirme </button>", "Veuillez confirmer !", {
        ...
        onShown: function(toast) {
          $("#confirmationRevertYes").click(function() {
            NProgress.start()
            Bloc.delete(data).then(async function(result) {
              ...
              vm.data_bloc = vm.data_bloc.filter(item => item.ID !== data.ID);
              vm.dtInstance.reloadData();
    ...
    $scope.check_all_data_input = async function(){
      var isDuplicate = vm.data_bloc.some(function(societe) {
  ...
  $scope.undoSelect = async function(){
    vm.data_bloc = vm.data_bloc.map(societe => {
  ...
  $scope.getSelectedIDs = async function(data) {
    ...
    $scope.toggleSelection = function (id) {
      ...
      /* if (!found) {
            vm.data_bloc.push({ id_sco_temp: id, selected: true });
        }    */
  };
    ...
    vm.dtColumns = [
      DTColumnBuilder.newColumn(null)
        ...
        DTColumnBuilder.newColumn("Nom").withTitle("Fermer").withOption("width", "100px"),
        DTColumnBuilder.newColumn("BLOC").withTitle("Bloc d'irrigation").withOption("width", "100px"),
        ...
        if(await $scope.validateData()){
                    ...
                    Bloc.multiadd({
                      fournisseurs :vm.jsonData
                        ...
                        vm.data_bloc.unshift(...e.data.inserted_data);
