...
/**
 * @ngdoc function
 * @name beeOneWebFrontApp.controller:GrandesculturesComposantesRendementCtrl
 * @description
 * # GrandesculturesComposantesRendementCtrl
 * Controller of the beeOneWebFrontApp
 */
angular.module('beeOneWebFrontApp')
  .controller('GrandesculturesComposantesRendementCtrl', function(
    $scope,
    DTOptionsBuilder,
    translatedwords,
    _url,
    ...
    var opsemisAccess = _.filter(permission.sous_modules_array, {
      ss_module: "composantes_rendement",
    });
    ...
    $scope.getdata = function(data) {
      return composantesRendement.get_composantes_rendement(data);
    };
    ...
    pc.detailsuividesstadesgc = function(data) {
      ...
      $q.all([
        composantesRendement.get_composantes_rendement_byid({
          ID: pc.expeditionByID.id
        }),
        composantesRendement.get_composantes_rendement_element_byid({
          ID: pc.expeditionByID.id
        })
      ]).then((values) => {
        pc.composantes_rendement_elements = values[0].data;
        pc.mgc_composantes_rendement_mgc_elements = values[1].data;
        NProgress.done();
      });
    ...
    pc.dtColumns = [
      ...
      .renderWith(function(data, type, full, meta) {
        return moment(full.date_constation).format("DD/MM/YYYY");
      }),
      ...
      DTColumnBuilder.newColumn("Stade_phenologique_actuel").withTitle(
        translatedwords.getTranslatedWord($translate("Stade actuel"))
      ),
      DTColumnBuilder.newColumn("rendement_estime")
      .withTitle(translatedwords.getTranslatedWord($translate("Rendement estimé (qx/ha)")))
      .renderWith(function(data, type, full, meta) {
        if (full.rendement_estime)
          return full.rendement_estime.toFixed(2);
        return 0;
      }),
      DTColumnBuilder.newColumn("rendement_theorique")
      .withTitle(translatedwords.getTranslatedWord($translate("Rendement théorique (qx/ha)")))
      .renderWith(function(data, type, full, meta) {
        if (full.rendement_theorique)
          return full.rendement_theorique.toFixed(2);
        return 0;
      }),
    ...
    pc.detailsorder = function(data) {
      pc.composantes_rendement_elements = [];
      pc.mgc_composantes_rendement_mgc_elements = [];
