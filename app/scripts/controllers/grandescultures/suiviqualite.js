...
/**
 * @ngdoc function
 * @name beeOneWebFrontApp.controller:GrandesculturesSuiviqualiteCtrl
 * @description
 * # GrandesculturesSuiviqualiteCtrl
 * Controller of the beeOneWebFrontApp
 */
angular.module('beeOneWebFrontApp')
  .controller('GrandesculturesSuiviqualiteCtrl', function(
    $scope,
    DTOptionsBuilder,
    translatedwords,
    _url, VarieteService
    ...
    toastr,
    campagneagricole, cultureService, familleculture,
    DTColumnBuilder,
    $mdDialog,
    $translate, portGreffe,
    $q,
    $compile,
    DTDefaultOptions,
    $cookies,
    ...
    pc.dtColumns = [
      ...
      DTColumnBuilder.newColumn("Famille").withTitle(
        translatedwords.getTranslatedWord($translate("Famille de culture"))
      ),
      ...
      DTColumnBuilder.newColumn("generation_semee").withTitle(
        translatedwords.getTranslatedWord($translate("Génération semée"))
      ),
      DTColumnBuilder.newColumn("type_production").withTitle(
        translatedwords.getTranslatedWord($translate("Type de production"))
      ),
      DTColumnBuilder.newColumn("generation_produite").withTitle(
        translatedwords.getTranslatedWord($translate("Génération produite"))
      ),
      DTColumnBuilder.newColumn("production_qx").withTitle(
        translatedwords.getTranslatedWord($translate("Production (qx)"))
      ),
      DTColumnBuilder.newColumn("realisation_epuration").withTitle(
        translatedwords.getTranslatedWord($translate("Réalisation épuration"))
      ),
    ...
    $scope.showAdvancedAdd = function(ev) {
      $mdDialog.show({
          ...
          templateUrl: '././views/templates/suiviqualite/Addsuiviqualite.html',
    ...
    function DialogControllerAdd($scope, $mdDialog) {
      ...
      $scope.TypeProduction = 1;
      $scope.RealisationEpuration = "Non";
      $scope.AgreageONSSA = "Non";
      $scope.somme_quantite_livree_agree = 0;
      $scope.moyenne_poids_specifique = 0;

      $scope.causes = [
      {name:"Faculté germinative" , title:"Faculté germinative"},
      {name:"Grains d''autres espèces" , title:"Grains d'autres espèces"},
      {name:"Mélange par les MH coriaces" , title:"Mélange par les MH coriaces"},
      {name:"PS inférieur à la norme" , title:"PS inférieur à la norme"},
      {name:"Grains charonconnés" , title:"Grains charonconnés"},
      {name:"Pureté spécifique" , title:"Pureté spécifique"},
      {name:"Humidité elevée" , title:"Humidité elevée"}];
      

      $scope.foodsPoidsSpecifiqueCertificatONSSA = [{
        quantite_livree_agree: null,
        poids_specifique: null
      }];
      ...
      $q.all([
        ...
        cultureService.getCultureByFerme(pc.IDFERME),
        familleculture.getAllfamilleculture()
      ]).then((values) => {
        $scope.varietes = values[0].data;
        $scope.portGreffes = values[1].data;
        $scope.cultures = values[2].data;
        $scope.famillecultures = values[3].data;
        NProgress.done();
      ...
      $scope.getCultureVarieteParcelle = function() {
        $scope.cultures = [];
        ...
        $q.all([
          cultureService.byFamille({
            Domaine: pc.IDFERME,
            Famille: ($scope.familleculturesel && $scope.familleculturesel.length > 0) ? $scope.familleculturesel : [0]
          }),
          VarieteService.ByFamilleCulture({
            Domaine: pc.IDFERME,
            Culture: ($scope.culturesel && $scope.culturesel.length > 0) ? $scope.culturesel : [0],
            Famille: ($scope.familleculturesel && $scope.familleculturesel.length > 0) ? $scope.familleculturesel : [0]
          }),
          parcellecultural.ByFamilleCultureVariete({
            Domaine: pc.IDFERME,
            Culture: ($scope.culturesel && $scope.culturesel.length > 0) ? $scope.culturesel : [0],
            Famille: ($scope.familleculturesel && $scope.familleculturesel.length > 0) ? $scope.familleculturesel : [0],
            Variete: ($scope.varietesel && $scope.varietesel.length > 0) ? $scope.varietesel : [0]
          })
        ]).then((values) => {
          ...
          $scope.varietesel = null;
          $scope.parcelleculturals = values[2].data;
          $scope.parcelleculturalsel = null;
          NProgress.done();
          $scope.letmeclick = true;
        });
      ...
      $scope.getVarieteParcelle = function() {
        $scope.varietes = [];
        $scope.varietesel = null;
        $scope.parcelleculturals = [];
        $scope.parcelleculturalsel = null;
        ...
        $q.all([
          VarieteService.ByFamilleCulture({
            Domaine: pc.IDFERME,
            Culture: ($scope.culturesel) ? $scope.culturesel : [0],
            Famille: ($scope.familleculturesel) ? $scope.familleculturesel : [0]
          }),
          parcellecultural.ByFamilleCultureVariete({
            Domaine: pc.IDFERME,
            Culture: ($scope.culturesel) ? $scope.culturesel : [0],
            Famille: ($scope.familleculturesel) ? $scope.familleculturesel : [0],
            Variete: ($scope.varietesel) ? $scope.varietesel : [0]
          })
        ]).then((values) => {
          $scope.varietes = values[0].data;
          $scope.varietesel = null;
      ...
      $scope.getlastproduct = function() {
        $scope.lastproduct = {};
        $q.all([
          suiviqualite.getlastProduct({
            Parcelle: $scope.parcelleculturalsel,
            ID_Societe: pc.IDSociete
          })
        ]).then((values) => {
          $scope.lastproduct = values[0].data;
          if ($scope.lastproduct.length == 0) {
            toastr.clear();
            toastr.error("Aucun produit trouvé", {
              closeButton: true
            });
          }
          $scope.lastproduct = ($scope.lastproduct.length > 0) ? $scope.lastproduct[0] : {};
          NProgress.done();
          $scope.letmeclick = true;
        });
      }
      ...
      $scope.cloneItem = async function(index) {
        if ($scope.foodsPoidsSpecifiqueCertificatONSSA[index].quantite_livree_agree && $scope.foodsPoidsSpecifiqueCertificatONSSA[index].poids_specifique) {
          $scope.foodsPoidsSpecifiqueCertificatONSSA.push({
            quantite_livree_agree: null,
            poids_specifique: null
          });
      ...
      $scope.removeItem = function(itemIndex) {
        $scope.foodsPoidsSpecifiqueCertificatONSSA.splice(itemIndex, 1);
        if ($scope.foodsPoidsSpecifiqueCertificatONSSA.length == 0) {
          $scope.foodsPoidsSpecifiqueCertificatONSSA.push({
            quantite_livree_agree: null,
            poids_specifique: null
          });
      ...
      $scope.setSomme = function() {
        var sumValue = _.sumBy($scope.foodsPoidsSpecifiqueCertificatONSSA, 'quantite_livree_agree');
        $scope.somme_quantite_livree_agree = sumValue;
        return (sumValue) ? sumValue : 0
      }

      $scope.setMoyenne = function() {
        var meanValue = _.sumBy($scope.foodsPoidsSpecifiqueCertificatONSSA, 'poids_specifique') / $scope.foodsPoidsSpecifiqueCertificatONSSA.length;
        $scope.moyenne_poids_specifique = meanValue;
        return parseFloat(meanValue.toFixed(2));
      }

      $scope.settaux_ecart = function(){
        try {
          $scope.taux_ecart = parseFloat(($scope.quantite_ecart / $scope.somme_quantite_livree_agree).toFixed(2))
       } catch (error) {
          $scope.taux_ecart= 0;
        }        
      }

      $scope.setdeclassementpartiel_pourcentage_declasse = function(){
        try {
          $scope.declassementpartiel_pourcentage_declasse = parseFloat(($scope.declassementpartiel_net_declasse / $scope.net_conditionne).toFixed(2))
       } catch (error) {
          $scope.declassementpartiel_pourcentage_declasse= 0;
        }
      }

      $scope.setnet_agree_pourcentage = function(){
        if($scope.net_agree_qx){
          $scope.declassementtotal_pourcentage_declasse = undefined;
          $scope.declassementtotal_cause = null;
        }
        try {
          $scope.net_agree_pourcentage = parseFloat(($scope.net_agree_qx / $scope.net_conditionne).toFixed(2));          
        $scope.setdeclassementpartiel_pourcentage_declasse();
       } catch (error) {
          $scope.net_agree_pourcentage= 0;          
        $scope.setdeclassementpartiel_pourcentage_declasse();
        }
      }
      ...
      $scope.Ajouter = async function() {
        ...
        if ($scope.date_previsionnelle_debut && $scope.parcelleculturalsel.length > 0 && $scope.type_de_taille && $scope.mode_de_taille && $scope.intensite_de_la_taille) {
          ...
          $scope.progress = false;
          toastr.clear();
          toastr.error("Apologies, but this action lacks both an API and corresponding database tables. 😈 ", {
            closeButton: true
          });
    ...
    function DialogControllerEdit($scope, $mdDialog, data) {
      ...
      $q.all([
        ...
        $scope.parcelleculturals = values[0].data;
        $scope.elementGc = values[1].data;
        $scope.qualiteStaderef = values[2].data;
        $scope.mesures = values[3].data;
