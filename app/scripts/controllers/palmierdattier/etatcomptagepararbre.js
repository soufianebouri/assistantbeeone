...
/**
 * @ngdoc function
 * @name beeOneWebFrontApp.controller:PalmierdattierEtatcomptagepararbreCtrl
 * @description
 * # PalmierdattierEtatcomptagepararbreCtrl
 * Controller of the beeOneWebFrontApp
 */
angular.module('beeOneWebFrontApp')
  .controller('PalmierdattierEtatcomptagepararbreCtrl', function($scope, $q, gestionPays, ressourcesHydriques, Arbre, translatedwords, toastr, elementcomptage, $translatePartialLoader, comptageArbre, $translate, $window, analyseQualitative, NiveauColorationService, pourcentageOuverture, $state, campagneagricole, domaine, parcellecultural, $cookies, savefilter) {
    NProgress.start();
    var pc = this;
    ...
    pc.loadSynthesisData = async () => {
      toastr.clear();
      if (pc.obj.ELEMENT.length >= 2) {
        $q.all([comptageArbre.getforEtatByArbre(pc.obj)]).then(async function(values) {
        ...
        toastr.error(await translatedwords.getTranslatedWord($translate("Veuillez choisir au moins deux éléments de comptage")), {
          closeButton: true
        });
      }
    }
    ...
    pc.arbre_change = function() {
      ...
      pc.obj.ARBRE = Arbre;
      pc.obj.ARBRE = (pc.Arbres.length == pc.obj.ARBRE.length) ? [0] : pc.obj.ARBRE
