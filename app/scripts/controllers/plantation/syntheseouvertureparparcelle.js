...
/**
 * @ngdoc function
 * @name beeOneWebFrontApp.controller:PlantationSyntheseouvertureparparcelleCtrl
 * @description
 * # PlantationSyntheseouvertureparparcelleCtrl
 * Controller of the beeOneWebFrontApp
 */
angular.module('beeOneWebFrontApp')
  .controller('PlantationSyntheseouvertureparparcelleCtrl', function($scope, translatedwords, $translatePartialLoader, $window, $translate, $q, pourcentageOuverture, $state, campagneagricole, domaine, parcellecultural, $cookies, savefilter) {
    NProgress.start();
    var pc = this;
    ...
    pc.loadSynthesisData = () => {
      $q.all([pourcentageOuverture.getParcelWithOutArbre(pc.obj), pourcentageOuverture.getParcelWithOrientationWithOutArbre(pc.obj)]).then(function(values) {
        pc.parcelWithOutArbre = values[0].data;
    ...
    $q.all([campagneagricole.getCampagneAgricole(),
      ...
      pourcentageOuverture.getParcelWithOutArbre(pc.obj),
      pourcentageOuverture.getParcelWithOrientationWithOutArbre(pc.obj)
    ]).then((values) => {

      pc.campagnes = values[0].data;
      pc.domaines = values[1].data;
      pc.parcellescultural = values[2].data;
      pc.parcelWithOutArbre = values[3].data;
    ...
    pc.printExcel = (e) => {
      $("#tableDataShow").table2excel({
        filename: "Synthèse de pourcentage d ouverture par parcelle.xls",
        preserveColors: true
      });
    };
