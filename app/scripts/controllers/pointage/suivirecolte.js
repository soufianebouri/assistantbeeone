...
/**
 * @ngdoc function
 * @name beeOneWebFrontApp.controller:PointageSuivirecolteCtrl
 * @description
 * # PointageSuivirecolteCtrl
 * Controller of the beeOneWebFrontApp
 */
angular.module('beeOneWebFrontApp')
  .controller('PointageSuivirecolteCtrl', function($scope, translatedwords, DTOptionsBuilder, $translatePartialLoader, $translate, $window, DTColumnBuilder, $q, $compile, suivirecolte, $state, DTDefaultOptions, $cookies, $templateCache) {
    var pc = this;
    pc.dtInstance = {};
    ...
    $scope.updatePointage = function(data) {
      return suivirecolte.getByFiltre(data);
    };
    ...
    pc.detailspointage = function(data) {
      ...
      suivirecolte.showCalibre({
        "IDPointage": pc.pointageByID.IDPointage
      }).then(function(res) {
        pc.callibres = res.data;
        NProgress.done();
      });

      suivirecolte.showParcelleByCalibre({
        "IDPointage": pc.pointageByID.IDPointage
      }).then(function(res) {
        pc.parcellebycallibres = res.data;
        NProgress.done();
      });
    ...
    pc.gettotalrowcalibre = function(data, IDTracabilite_recolte_shosen) {
      pc.totalrowcalibre = 0;
      angular.forEach(data, function(parcellestracabilite) {
        if (parcellestracabilite.IDTracabilite_recolte == IDTracabilite_recolte_shosen) {
          pc.totalrowcalibre += parseFloat(parcellestracabilite.Quantite);
        }
      })
      return pc.totalrowcalibre;
    }

    pc.gettotalallcalibre = function(data) {
      pc.totalallcalibre = 0;
      angular.forEach(data, function(parcellestracabilite) {
        pc.totalallcalibre += parseFloat(parcellestracabilite.Quantite);
      })
      return pc.totalallcalibre;
    }
      ...
      .withButtons([{
...

        {
          ...
          buttons: [{
            ...
            {
              text: translatedwords.getTranslatedWord($translate("Horaire")),
              action: function(e, dt, node, config) {
            ...
            {
              text: translatedwords.getTranslatedWord($translate("à l'unité")),
              action: function(e, dt, node, config) {
    ...
    pc.dtColumns = [
      ...
      DTColumnBuilder.newColumn('Cout_Totale').withTitle(translatedwords.getTranslatedWord($translate("Coût"))).renderWith(function(data, type, full, meta) {
        ...
        return 'ID=' + full.IDPointage + ' <p align="right">' + Cout_Totale + '</p>';
      })
    ];
