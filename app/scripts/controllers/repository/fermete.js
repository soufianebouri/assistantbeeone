...
/**
 * @ngdoc function
 * @name beeOneWebFrontApp.controller:RepositoryFermeteCtrl
 * @description
 * # RepositoryFermeteCtrl
 * Controller of the beeOneWebFrontApp
 */
angular.module('beeOneWebFrontApp')
  .controller('RepositoryFermeteCtrl', function($scope,
    $compile,
    $uibModal,
    _url,
    DTOptionsBuilder,
    ...
    $q,
    fermete, $mdDialog, $translatePartialLoader, $translate, $window,
    ...
    pc.dtOptions = DTOptionsBuilder.fromFnPromise(function() {
        var defer = $q.defer();
        fermete.getallFruit().then(function(result) {
          NProgress.done();
    ...
    function DialogControllerAddFermete($scope, $mdDialog) {
      ...
      $scope.AjouterFermete = async function() {
        ...
        if ($scope.Code) {
          ...
          $scope.progress = true;
          fermete.createFruit(pc.objNewFermete).then(async e => {
    ...
    function DialogControllerEdit($scope, $mdDialog, datafermete) {
      ...
      $scope.ModiferFermete = async function() {
        ...
        if ($scope.data.Code) {
          ...
          $scope.progress = true;
          fermete.updateFruit(pc.objEdit).then(async e => {
            if (e.data[0].message == "ajout reussi") {
              //validate success
              toastr.clear();
