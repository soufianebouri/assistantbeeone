...
/**
 * @ngdoc function
 * @name beeOneWebFrontApp.controller:RepositoryGenerationCtrl
 * @description
 * # RepositoryGenerationCtrl
 * Controller of the beeOneWebFrontApp
 */
angular.module('beeOneWebFrontApp')
  .controller('RepositoryGenerationCtrl', function($scope,
    $compile,
    $uibModal,
    _url,
    DTOptionsBuilder,
    ...
    $q,
    generation, $mdDialog, $translatePartialLoader, $translate, $window,
    ...
    pc.dtOptions = DTOptionsBuilder.fromFnPromise(function() {
        var defer = $q.defer();
        generation.getall().then(function(result) {
          NProgress.done();
    ...
    pc.dtColumns = [
      ...
      DTColumnBuilder.newColumn('genereation').withTitle(translatedwords.getTranslatedWord($translate("Génération"))),
    ...
    //AddGeneration
    pc.AddGeneration = function() {
      $mdDialog.show({
          controller: DialogControllerAddGeneration,
          templateUrl: '././views/templates/genaration/AddGeneration.html',
    ...
    //add code
    function DialogControllerAddGeneration($scope, $mdDialog) {
      $scope.progress = false;
      $scope.AjouterGeneration = async function() {
        toastr.clear();
        if ($scope.genereation) {
          pc.objNewGeneration = {
            "genereation": $scope.genereation,
            "Reference": ($scope.Reference) ? $scope.Reference : ""
          }
          $scope.progress = true;
          generation.create(pc.objNewGeneration).then(async e => {
            if (e.data[0].message == "ajout reussi") {
              ...
              if (e.data[0].description.includes("duplicate key")) {
                $scope.progress = false;
                toastr.error(await translatedwords.getTranslatedWord($translate("Veuillez noter que cette Génération existe déjà !")), {
                  closeButton: true
                });
              } else {
                $scope.progress = false;
      ...
      $scope.hideGeneration = function() {
        $mdDialog.hide();
        document.getElementsByClassName('left_col')[0].style.zIndex = 999999;
      };

      $scope.AnnulerGeneration = function() {
        $mdDialog.cancel();
        document.getElementsByClassName('left_col')[0].style.zIndex = 999999;
      };
    ...
    //edit
    pc.edit = function(datageneration) {
      $scope.showAdvancedEdit("ev", datageneration);
    }

    $scope.showAdvancedEdit = function(ev, datageneration) {
      $mdDialog.show({
          controller: DialogControllerEdit,
          templateUrl: '././views/templates/genaration/EditGeneration.html',
          parent: angular.element(document.body),
          targetEvent: ev,
          clickOutsideToClose: false,
          locals: {
    ...
    function DialogControllerEdit($scope, $mdDialog, datageneration) {
      $scope.data = datageneration;
      $scope.progress = false;
      $scope.ModiferGeneration = async function() {
        toastr.clear();
        if ($scope.data.genereation) {
          pc.objEdit = {
            "genereation": $scope.data.genereation,
            "Reference": ($scope.data.Reference) ? $scope.data.Reference : "",
            "IDGeneration": $scope.data.IDGeneration
          }
          $scope.progress = true;
          generation.update(pc.objEdit).then(async e => {
            if (e.data[0].message == "ajout reussi") {
              //validate success
              toastr.clear();
    ...
    pc.delete = async function(c) {
      pc.IDGeneration = c.IDGeneration;
      toastr.clear();
