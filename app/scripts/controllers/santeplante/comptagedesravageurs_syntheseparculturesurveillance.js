...
/**
 * @ngdoc function
 * @name beeOneWebFrontApp.controller:SanteplanteComptagedesravageursSyntheseparculturesurveillanceCtrl
 * @description
 * # SanteplanteComptagedesravageursSyntheseparculturesurveillanceCtrl
 * Controller of the beeOneWebFrontApp
 */
angular.module('beeOneWebFrontApp')
  .controller('SanteplanteComptagedesravageursSyntheseparculturesurveillanceCtrl', function($scope, translatedwords, $window, DTOptionsBuilder, $translatePartialLoader, $translate, DTColumnBuilder, $q, $compile, $state, DTDefaultOptions, $cookies, comptagedesravageursSyntheseparculturesurveillance, parcellecultural, Cible, _url, cultureService, savefilter) {
    ...
    $scope.LoadCible = Cible.getCible(_url).then(function(res) {
      pc.cibes = res.data;
    });
    ...
    //load Parcelle cultural list by domaine
    $scope.LoadCulture = cultureService.getCulture(_url).then(function(res) {
      pc.cultures = res.data;
    });
    ...
    $q.all([$scope.LoadParcelleCultural, $scope.LoadCible, $scope.LoadCulture]).then(function(values) {
      setTimeout(function() {
        $("#Parcelle").selectpicker('refresh');
        $("#Cible").selectpicker('refresh');
        $("#Culture").selectpicker('refresh');
    ...
    pc.loadData = () => {
      $q.all([comptagedesravageursSyntheseparculturesurveillance.getDistinctedCible(pc.obj), comptagedesravageursSyntheseparculturesurveillance.getArbreParcelle(pc.obj), comptagedesravageursSyntheseparculturesurveillance.getNbrInsectes(pc.obj)]).then(function(values) {
    ...
    //by culture
    $scope.culture_change = function() {
      if ($scope.culture.culture === null || $scope.culture.culture === "" || $scope.culture.culture === undefined || $scope.culture.culture === 0 || $scope.culture.culture === "0" || !$scope.culture.culture || $scope.culture.culture.length === 0 || $scope.culture.culture.includes(0)) {
        $scope.culture_sel = [0];
      } else {
        $scope.culture_sel = $scope.culture.culture;
      }
      pc.obj.CULTURE = $scope.culture_sel;
      pc.loadData();
    };
    ...
    pc.printExcel = (e) => {
      $("#tableDataShow").table2excel({
        filename: "Synthèse de surveillance par culture.xls",
        preserveColors: true
      });
    };
