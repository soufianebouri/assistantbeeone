...
/**
 * @ngdoc function
 * @name beeOneWebFrontApp.controller:ModalGestionpaysCtrl
 * @description
 * # ModalGestionpaysCtrl
 * Controller of the beeOneWebFrontApp
 */
angular.module('beeOneWebFrontApp')
  .controller('ModalGestionpaysCtrl', function($uibModalInstance, translatedwords, data, $base64, $scope, gestionPays, $q, $translatePartialLoader, $translate, $window, $filter) {
    var pc = this;
    pc.data = data;
    ...
    if (pc.data.action == "insert" || pc.data.action == "update") {
      ...
      pc.submitForm = function(isValid) {
        if (isValid) {
          ...
          if (pc.data.action == "update") {
            console.log("update");
            gestionPays.updatePays(pc.data.baseUrl, pc.obj).then(function(resp) {
          ...
          gestionPays.pushPays(pc.data.baseUrl, pc.obj).then(function(resp) {
            if (resp.data[0].message == "ajout reussi") { //validate success
              $uibModalInstance.close('insert');
            } else {
              pc.errPushInsert += "an error occured " + resp.data[0].description;
