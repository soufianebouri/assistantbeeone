...
/**
 * @ngdoc function
 * @name beeOneWebFrontApp.controller:ModalElimiationCtrl
 * @description
 * # ModalElimiationCtrl
 * Controller of the beeOneWebFrontApp
 */
angular.module('beeOneWebFrontApp')
  .controller('ModalElimiationCtrl', function($uibModalInstance, data, $base64, translatedwords, $scope, $window, $translatePartialLoader, $translate, LieuElimination, $q, $filter) {
    var pc = this;
    pc.data = data;
    ...
    if (pc.data.action == "insert" || pc.data.action == "update") {
      ...
      pc.submitForm = function(isValid) {
        if (isValid) {
          ...
          if (pc.data.action == "update") {
            console.log("update");
            LieuElimination.updateLieuElimination(pc.data.baseUrl, pc.obj).then(function(resp) {
          ...
          LieuElimination.pushLieuElimination(pc.data.baseUrl, pc.obj).then(function(resp) {
            if (resp.data[0].message == "ajout reussi") { //validate success
              $uibModalInstance.close('insert');
            } else {
              pc.errPushInsert += "an error occured " + resp.data[0].description;
