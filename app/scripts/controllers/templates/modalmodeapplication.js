...
/**
 * @ngdoc function
 * @name beeOneWebFrontApp.controller:ModalModeApplicationCtrl
 * @description
 * # ModalModeApplicationCtrl
 * Controller of the beeOneWebFrontApp
 */
angular.module('beeOneWebFrontApp')
  .controller('ModalModeApplicationCtrl', function($uibModalInstance, translatedwords, data, $base64, $scope, $window, $translatePartialLoader, $translate, ModeApplication, $q, $filter) {
    var pc = this;
    pc.data = data;
    ...
    if (pc.data.action == "insert" || pc.data.action == "update") {
      ...
      pc.submitForm = function(isValid) {
        if (isValid) {
          ...
          if (pc.data.action == "update") {
            console.log("update");
            ModeApplication.updateModeApplication(pc.data.baseUrl, pc.obj).then(function(resp) {
          ...
          ModeApplication.pushModeApplication(pc.data.baseUrl, pc.obj).then(function(resp) {
            if (resp.data[0].message == "ajout reussi") { //validate success
              $uibModalInstance.close('insert');
            } else {
              pc.errPushInsert += "an error occured " + resp.data[0].description;
