...
/**
 * @ngdoc function
 * @name beeOneWebFrontApp.controller:ModalConformiteCtrl
 * @description
 * # ModalConformiteCtrl
 * Controller of the beeOneWebFrontApp
 */
angular.module('beeOneWebFrontApp')
  .controller('ModalConformiteCtrl', function($uibModalInstance, data, translatedwords, $base64, $scope, $window, $translatePartialLoader, $translate, NiveauConformite, $q, $filter) {
    var pc = this;
    pc.data = data;
    ...
    if (pc.data.action == "insert" || pc.data.action == "update") {
      ...
      pc.submitForm = function(isValid) {
        if (isValid) {
          ...
          if (pc.data.action == "update") {
            console.log("update");
            NiveauConformite.updateNiveauConformite(pc.data.baseUrl, pc.obj).then(function(resp) {
          ...
          NiveauConformite.pushNiveauConformite(pc.data.baseUrl, pc.obj).then(function(resp) {
            if (resp.data[0].message == "ajout reussi") { //validate success
              $uibModalInstance.close('insert');
            } else {
              pc.errPushInsert += "an error occured " + resp.data[0].description;
