    ...
    $scope.saveData = () => {
      ...
      angular.forEach(tabProfile, function(profile, key) {
        ...
        angular.forEach(tabeDates, function(value, key) {
          rqt += "(" + profile.id + ",'" + moment(new Date(value)).format('YYYY-MM-DD') + "','" + moment().format('YYYY-MM-DD') + "','" + $cookies.getObject('beeoneAssistant').assistUser.Nom + " " + $cookies.getObject('beeoneAssistant').assistUser.Prenom + "'),";
        });
      });
